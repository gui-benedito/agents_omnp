...
async def main():
    ...
    terminal_server = MCPServerStdio(
        params={
            "command": "npx",
    ...
    await browser_server.connect()
    await terminal_server.connect()

    agent = Agent(
        name="Assistente pessoal de navegador e terminal",
        instructions="Você é um assistente prestativo com acesso ao meu navegador e ao meu terminal, que usa o sistema windows. Antes de rodar qualquer comando no terminal, rode um pwd para saber onde vc está. Só faça coisas dentro do diretório atual.",
        model='gpt-5-nano',
        mcp_servers=[
            browser_server,
            terminal_server
